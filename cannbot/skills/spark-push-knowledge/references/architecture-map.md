...
| 问题领域 | 首选文档 | 当前代码真源 |
|---|---|---|
| 总体架构、组件边界、启动入口 | `README.md`、`scripts/00_prepare_and_run.md` | `CMakeLists.txt`、各组件 `main.cpp` |
| WebSocket 接入、ACK、断线补偿 | `docs/reliability-optimization.md` | `comet/comet_server.cpp`、`comet/comet_grpc_service.cpp` |
| 序号、幂等、会话历史、AI 请求编排 | `docs/reliability-optimization.md`、`docs/hermes-integration.md` | `logic/grpc_service.cpp`、`logic/redis_store.cpp`、`logic/conversation_store.cpp` |
| Kafka 消费、持久化、Job→Comet 推送 | `docs/reliability-optimization.md` | `job/service.cpp`、`common/kafka_consumer.cpp` |
...
| Pi ↔ IM 架构审查、通用事件、会话队列与隔离方案 | `docs/pi-im-architecture-review.md`（2026-09-06 审查快照；方案尚未实施） | `cannbot/scripts/pi_gateway.py`、`hermes_bridge/`、`logic/grpc_service.cpp`、`proto/spark_push.proto` |
...
| 多 Agent 选择、所有者配置和 Windows technical 隔离接入 | `docs/multi-agent-integration.md` | `cannbot/scripts/agent_router.py`、`cannbot/scripts/hermes_adapter.py`、`cannbot/scripts/windows_agent_transport.py` |
