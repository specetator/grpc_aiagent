...
// 功能：为会话追加消息，写入 MySQL/Redis
// msg_id 由服务端生成（Redis 自增），msg_seq 是从 msg_id 提取的严格递增序号
bool ConversationStore::AppendMessage(const Session& session,
    ...
    // 验证 msg_seq 与 msg_id 的一致性
    // 从 msg_id 解析出序号部分: msgid:1:2-123 -> 123
    int64_t msg_id_seq = 0;
    size_t dash_pos = msg_id.rfind('-');
    if (dash_pos != std::string::npos && dash_pos + 1 < msg_id.size()) {
        try {
            msg_id_seq = std::stoll(msg_id.substr(dash_pos + 1));
        } catch (...) {
            LOG_ERROR << "Failed to parse msg_id sequence from: " << msg_id;
        }
    }

    // msg_seq 应该等于从 msg_id 提取的序号
    if (msg_seq != msg_id_seq) {
        LOG_WARN << "msg_seq mismatch: provided=" << msg_seq 
                 << ", extracted=" << msg_id_seq << ", using extracted value";
        msg_seq = msg_id_seq;  // 使用从 msg_id 提取的序号
    }
    ...
    // 更新 session 的 last_msg_seq（使用 msg_id 的序号部分）
    if (session_dao_ && msg_id_seq > 0) {
        std::string update_err;
        if (!session_dao_->UpdateLastMsgSeq(session.id, msg_id_seq,
                                            &update_err)) {
            LOG_ERROR << "UpdateLastMsgSeq failed: " << update_err;
        }
    }

    if (redis_store_ && msg_id_seq > 0) {
        // 将最新 seq 写入 Redis，便于未读计算
        if (!redis_store_->SetSessionLastSeq(session.id, msg_id_seq)) {
