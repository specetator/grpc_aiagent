...
本节课是基于 C++ 的实时消息推送系统的**认证子集**教学材料，专注于用户身份认证和 WebSocket 连接管理的核心功能。
...
- `02_auth_subset/CMakeLists.txt`：顶层构建配置
...
- 顶层：`02_auth_subset/CMakeLists.txt` 统一拉起子模块
...
```mermaid
graph TD
    A[02_auth_subset/CMakeLists]:::top --> B[muduo/base]:::lib
    A --> C[muduo/net]:::lib
    A --> D[common]:::lib
    A --> E[proto]:::gen
    A --> F[logic]:::svc
...
```bash
cd spark_push/02_auth_subset/build
./logic/logic_server -c ../conf/logic.conf
```

**终端 2：启动 Comet**

```bash
cd /home/alientek/spark_push/02_auth_subset
./comet/comet_server -c ../conf/comet.conf
```
...
注意：请将 v1.13.0 替换为最新版本号（截至 2025 年，最新版可能是 v1.13.0 或更高）。

# 验证安装：
websocat --version

# 连接 WebSocket（替换 YOUR_TOKEN）
websocat "ws://localhost:9000/ws?token=tk-1-1701234567"
```
...
```mermaid
sequenceDiagram
    participant GS as LogicServiceImpl<br/>(grpc_service.cpp)
    participant RS as RedisStore<br/>(redis_store.cpp)
    participant Pool as RedisPool<br/>(连接池)
    participant Redis as Redis Server
    
    Note over GS: VerifyToken 方法内
    GS->>GS: GetUserIdByToken(token) 成功
    GS->>GS: 获得 user_id
    
    GS->>RS: AddRoute(user_id, comet_id)
    activate RS
    
    RS->>Pool: Acquire() 获取连接
    activate Pool
    Pool-->>RS: guard(redisContext*)
    deactivate Pool
    Note over RS: RAII 自动管理连接<br/>析构时归还连接池
    
    RS->>Redis: SADD route:user:{user_id} {comet_id}
    Note right of Redis: Key: route:user:100000000001<br/>Value: {"comet-1"}
    
    Redis-->>RS: reply->type (SUCCESS)
    
    RS->>RS: 检查 reply->type
    RS->>RS: freeReplyObject(reply)
    RS->>Pool: guard 析构<br/>归还连接
    
    RS-->>GS: return true
    deactivate RS
    
    GS->>GS: SetError(0, "ok")
    GS-->>GS: return grpc::Status::OK
```

**Redis 数据结构示例**：

```
用户 100000000001 在 comet-1 上建立连接：
┌─────────────────────────────────────┐
│ Redis Key: route:user:100000000001  │
│ Type: SET                           │
│ Value: {"comet-1"}                  │
└─────────────────────────────────────┘

同一用户又在 comet-2 上建立连接（多设备）：
┌─────────────────────────────────────┐
│ Redis Key: route:user:100000000001  │
│ Type: SET                           │
│ Value: {"comet-1", "comet-2"}       │
└─────────────────────────────────────┘

SADD 命令特性：
• 自动去重（同一 comet_id 重复添加无影响）
• 原子操作（线程安全）
• 支持多值存储（一个用户多个设备）
```
...
```mermaid
graph TD
    A[Comet 调用 gRPC<br/>VerifyToken] --> B[LogicServiceImpl::VerifyToken<br/>grpc_service.cpp:17-35]
    
    B --> C{redis_store_ != nullptr?}
    C -->|否| D[返回错误 500<br/>redis store not initialized]
    C -->|是| E[GetUserIdByToken<br/>redis_store.cpp:30-50]
    
    E --> F{Token 存在且有效?}
    F -->|否| G[返回错误 401<br/>invalid or expired token]
    F -->|是| H[获得 user_id]
    
    H --> I[AddRoute<br/>redis_store.cpp:53-68]
    
    I --> J[从连接池获取 Redis 连接<br/>pool_->Acquire]
    J --> K{ctx != nullptr?}
    K -->|否| L[返回 false]
    K -->|是| M[执行 redisCommand<br/>SADD route:user:%lld %s]
    
    M --> N{reply != nullptr?}
    N -->|否| O[LOG_ERROR<br/>返回 false]
    N -->|是| P{reply->type != ERROR?}
    
    P -->|否| Q[返回 false]
    P -->|是| R[freeReplyObject<br/>返回 true]
    
    R --> S[response.set_user_id<br/>SetError code=0]
    S --> T[返回 grpc::Status::OK]
...
```ini
# comet TCP 监听（客户端长连接，对外暴露）
listen_addr=0.0.0.0
listen_port=9000

# comet 的 gRPC 端口及 ID（供 logic/job 等内部服务调用）
comet_grpc_port=9105
comet_id=comet-1

# logic 服务 gRPC 地址（指向 logic 的 listen_port）
...
### 10.7.2 配置加载实现（`common/config.cpp`）
