      ...
      systemInspector_(new SystemInspector)
{
  ...
  server_.setHttpCallback(std::bind(&Inspector::onRequest, this, _1, _2, _3));
...
bool Inspector::onRequest(const TcpConnectionPtr& conn, const HttpRequest& req, HttpResponse* resp)
{
  if (req.path() == "/")
  {
    string result;
