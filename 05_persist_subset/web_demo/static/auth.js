// 前端工具：提供 MD5(hash) 与简单的 session 存储。
// 注意：仅用于 demo。生产环境务必使用 HTTPS 与更安全的密码学方案。

/* eslint-disable */
// 经典 MD5 实现（压缩版），与仓库根目录 web_demo/static/auth.js 保持一致。
...
const Auth = {
  KEY_TOKEN: 'spark04_token',
  KEY_UID: 'spark04_uid',
  KEY_NAME: 'spark04_name',

  set: function (uid, token, name) {
    localStorage.setItem(this.KEY_UID, String(uid));
  ...
  get: function () {
    ...
    if (uid && token) return { user_id: uid, token, name };
    return null;
  },
