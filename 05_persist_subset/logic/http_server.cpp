...
HttpApiServer::HttpApiServer(EventLoop* loop, const InetAddress& listenAddr,
                             ...
                             RoomStore* room_store, MessageDao* message_dao,
...
void HttpApiServer::onRequest(const HttpRequest& req, HttpResponse* resp) {
    ...
    static const std::unordered_map<std::string, Handler> kRouteTable = {
        ...
        {"/api/message/history", &HttpApiServer::handleMessageHistory},
...
void HttpApiServer::handleRoomSendMessage(const HttpRequest& req,
    ...
    if (!message_dao_) {
        WriteJson(resp, 500, "message dao not initialized");
        return;
    }
    {
        PersistMessage pm;
        ...
        pm.msg_seq = msg_seq;
        pm.sender_id = from_user;
        pm.timestamp_ms = now_ms;
        pm.msg_type = msg_type;
        pm.content_json = final_json;
        pm.client_msg_id = client_msg_id;
        std::string err;
        if (!message_dao_->Insert(pm, &err)) {
            WriteJson(resp, 500, "persist message failed: " + err);
            return;
        }
    }
...
// 分页拉取历史消息（持久化回溯）
void HttpApiServer::handleMessageHistory(const HttpRequest& req,
                                         HttpResponse* resp) {
    if (req.method() != HttpRequest::kPost) {
        WriteJson(resp, 405, "only POST allowed", "{}",
                  HttpResponse::k400BadRequest);
    ...
    try {
        ...
        if (j.contains("limit")) limit = j.at("limit").get<int>();
    } catch (const nlohmann::json::exception&) {
    ...
    // 会话合法性校验：单聊必须包含当前用户；房间需在成员内
    if (session_id.rfind("s_", 0) == 0) {
        auto pos = session_id.find(':');
        if (pos == std::string::npos) {
            WriteJson(resp, 400, "invalid session_id format", "{}",
                      HttpResponse::k400BadRequest);
            return;
        }
        int64_t uid1 = 0, uid2 = 0;
        try {
            uid1 = std::stoll(session_id.substr(2, pos - 2));
            uid2 = std::stoll(session_id.substr(pos + 1));
        } catch (...) {
            WriteJson(resp, 400, "invalid session_id format", "{}",
                      HttpResponse::k400BadRequest);
            return;
        }
        if (user_id != uid1 && user_id != uid2) {
            WriteJson(resp, 403, "session not belong to user", "{}",
                      HttpResponse::k403Forbidden);
            return;
        }
    } else if (session_id.rfind("r_", 0) == 0) {
        int64_t room_id = 0;
        try {
            room_id = std::stoll(session_id.substr(2));
        } catch (...) {
            WriteJson(resp, 400, "invalid session_id format", "{}",
                      HttpResponse::k400BadRequest);
            return;
        }
        if (room_id > 0 && room_store_) {
            bool is_in = false;
            if (!room_store_->IsUserInRoom(user_id, room_id, &is_in, nullptr) ||
                !is_in) {
                WriteJson(resp, 403, "user not in room", "{}",
                          HttpResponse::k403Forbidden);
                return;
            }
        }
    ...
    std::vector<PersistMessage> msgs;
    std::string err;
    if (!message_dao_->List(session_id, anchor_seq, limit, &msgs, &err)) {
        WriteJson(resp, 500, "list messages failed: " + err);
        return;
    }
    ...
    for (auto it = msgs.rbegin(); it != msgs.rend(); ++it) {
        ...
        item["msg_seq"] = m.msg_seq;
        item["sender_id"] = m.sender_id;
        item["timestamp_ms"] = m.timestamp_ms;
        item["msg_type"] = m.msg_type;
        item["client_msg_id"] = m.client_msg_id;
        item["content_json"] = m.content_json;
        arr.push_back(item);
    }
    nlohmann::json data;
    data["messages"] = arr;
