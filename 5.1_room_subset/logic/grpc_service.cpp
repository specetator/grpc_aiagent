...
LogicServiceImpl::LogicServiceImpl(UserDao* user_dao, RoomDao* room_dao,
...
::grpc::Status LogicServiceImpl::VerifyToken(
    ...
    LOG_DEBUG << "VerifyToken called with token: " << request->token()
...
::grpc::Status LogicServiceImpl::SendUpstreamMessage(
    ...
    // 支持 single_chat 和 room 两种类型
    if (target_type != "single_chat" && target_type != "room") {
        SetError(response->mutable_error(), 400,
                 "target_type must be single_chat or room");
        return ::grpc::Status::OK;
    }
    ...
    if (target_type == "single_chat") {
        ...
        std::string msg_id;
        if (redis_store_ &&
            redis_store_->NextSingleMsgId(user1, user2, &msg_seq)) {
        ...
        // LOG_INFO << "Allocated msg_id " << msg_id << ", session_id "
        //          << session_id;
    ...
    if (target_type == "room") {
        ...
        // 步骤1：检查房间是否存在（可选：性能考虑可跳过）
        // Room room_info;
        // std::string err;
        // if (room_dao_ && !room_dao_->GetRoom(room_id, &room_info, &err)) {
        //     SetError(response->mutable_error(), 404, "room not found");
        //     return ::grpc::Status::OK;
        // }

        // 步骤2：成员校验（建议做：发送者必须是房间成员）
        if (room_dao_) {
            std::string err;
            if (!room_dao_->IsMember(room_id, from_user, &err)) {
                SetError(response->mutable_error(), 403,
                         "sender is not room member");
                LOG_WARN << "Room message rejected: user " << from_user
                         << " not member of room " << room_id;
                return ::grpc::Status::OK;
            }
        }
        ...
        if (redis_store_ && redis_store_->NextRoomMsgId(room_id, &msg_seq)) {
            msg_id = "room_msgid:" + std::to_string(room_id) + "-" +
                     std::to_string(msg_seq);
        } else {
            SetError(response->mutable_error(), 500,
                     "alloc room msg_seq failed");
            return ::grpc::Status::OK;
        }

        // LOG_INFO << "Allocated room msg_id " << msg_id << ", room_id=" << room_id;
        ...
        std::string final_json = request->content_json();
        std::string msg_type =
            request->msg_type().empty() ? "room" : request->msg_type();
        try {
            auto j = nlohmann::json::parse(final_json);
            // 最终协议：清理客户端自带的“旧字段/无关字段”，避免透传到下行
            j.erase("timestamp");
            j.erase("client_timestamp_ms");
            j.erase("to_user_id");
            j.erase("room_id");
            j["msg_id"] = msg_id;
            j["msg_seq"] = msg_seq;
        ...
        ChatMessage* cm = response->mutable_message();
        cm->set_msg_id(msg_id);
        cm->set_session_id("room:" + std::to_string(room_id));  // 房间会话 ID
        cm->set_msg_seq(msg_seq);
        cm->set_sender_id(from_user);
        cm->set_timestamp_ms(now_ms);
        cm->set_msg_type(msg_type);
        ...
        // 步骤7：检查 Kafka 生产者是否可用
        if (!push_producer_) {
            LOG_ERROR << "Kafka producer not ready for room message";
            return ::grpc::Status::OK;
        }

        // 步骤8：调用房间消息后置处理函数（精准投递）
        // 使用 lambda 作为回源函数，Redis miss 时查询 MySQL
        auto fallback = [this](int64_t rid,
                               std::vector<int64_t>* uids) -> bool {
            if (!room_dao_) return false;
            std::string err;
            return room_dao_->GetRoomMembers(rid, uids, &err);
        };
        PostProcessRoomMessage(room_id, *cm, redis_store_, push_producer_,
                               fallback);
    ...
    // 不应该到达这里
    SetError(response->mutable_error(), 500, "internal error");
    return ::grpc::Status::OK;
}
