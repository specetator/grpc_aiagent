...
bool ExtractHeader(const std::string &req, const std::string &header_name, std::string *value) {
    ...
    while (trimmed_end > pos && (req[trimmed_end - 1] == ' ' || req[trimmed_end - 1] == '\t' ||
...
CometServer::CometServer(EventLoop *loop, const Config &cfg)
    : server_(loop, muduo::net::InetAddress(cfg.listen_port), "comet_server"), loop_(loop) {
    ...
    auto channel = grpc::CreateChannel(cfg.logic_grpc_target, grpc::InsecureChannelCredentials());
    ...
    server_.setMessageCallback(std::bind(&CometServer::OnMessage, this, std::placeholders::_1,
...
// - 用户不在线时静默忽略，不返回错误
// ============================================================================
void CometServer::PushToUsers(const ChatMessage &msg, const std::vector<int64_t> &user_ids) {
    // ========================================================================
    ...
    std::string payload = msg.content_json().empty() ? ("{\"msg_id\":\"" + msg.msg_id() + "\"}")
                                                     : msg.content_json();
...
void CometServer::OnConnection(const TcpConnectionPtr &conn) {
    ...
    if (conn->connected()) {
        ...
        ctx.last_active_ms = std::chrono::duration_cast<std::chrono::milliseconds>(
...
void CometServer::HandleHandshake(const TcpConnectionPtr &conn, Buffer *buf) {
    ...
    const char *end = static_cast<const char *>(memmem(data, buf->readableBytes(), crlf2, 4));
    if (!end) {
    ...
    if (!status.ok() || vrep.error().code() != 0) {
        ...
        std::string msg = status.ok() ? vrep.error().message() : status.error_message();
    ...
    // 格式：user_connections:<user_id> -> {<conn_id>: <comet_id>}
    RegisterUserOnline(user_id, conn_id);

    // 记录日志：握手成功
    LOG_INFO << "WebSocket handshake done, user_id=" << user_id << " conn_id=" << conn_id;
}
...
// ```
// ============================================================================
void CometServer::HandleWebSocketFrame(const TcpConnectionPtr &conn, Buffer *buf,
                                       ConnContext &ctx) {
    // ========================================================================
    // 循环处理缓冲区中的所有完整帧
    // ========================================================================
    ...
    while (buf->readableBytes() >= 2) {  // 至少需要 2 字节（帧头的前两个字节）
        ...
        // 步骤 1: 解析帧头的前两个字节
        // ====================================================================
        const unsigned char *data = reinterpret_cast<const unsigned char *>(buf->peek());
        ...
        if (!fin || !masked) {
            // 不符合协议要求：关闭连接
            LOG_WARN << "Close connection from user " << ctx.user_id << " conn_id=" << ctx.conn_id
        ...
        if (payloadLen == 126) {
            // 中等长度：使用 16 位扩展字段
            if (buf->readableBytes() < headerLen + 2) return;  // 数据不足，等待更多
            const unsigned char *p = data + headerLen;
            payloadLen = (p[0] << 8) | p[1];  // 网络字节序（大端）转主机字节序
            headerLen += 2;
        } else if (payloadLen == 127) {
            // 长消息：使用 64 位扩展字段
            if (buf->readableBytes() < headerLen + 8) return;  // 数据不足，等待更多
            payloadLen = 0;
            const unsigned char *p = data + headerLen;
            // 逐字节读取（网络字节序）
            for (int i = 0; i < 8; ++i) {
        ...
        if ((opcode == 0x8 || opcode == 0x9 || opcode == 0xA) && payloadLen > 125) {
            LOG_WARN << "Close connection from user " << ctx.user_id << " conn_id=" << ctx.conn_id
                     << " due to control frame payload too large"
                     << " (opcode=" << static_cast<int>(opcode) << ", payload_len=" << payloadLen
                     << ")";
            conn->shutdown();
            return;
        }
        ...
        if (opcode == 0x8) {
            ...
            //      但简化实现中直接关闭 TCP 连接
            LOG_WARN << "Recv close frame from user " << ctx.user_id << " conn_id=" << ctx.conn_id;
            conn->shutdown();
            return;  // 关闭连接后不再处理后续帧
            ...
            frame.push_back(static_cast<char>(0x8A));        // FIN=1, Opcode=0xA (Pong)
            frame.push_back(static_cast<char>(payloadLen));  // 负载长度（服务端无需掩码）
            ...
            LOG_INFO << "Recv ping from user " << ctx.user_id << " conn_id=" << ctx.conn_id
                     << ", replied pong";
            ...
            LOG_INFO << "Recv pong from user " << ctx.user_id << " conn_id=" << ctx.conn_id;
            ...
            LOG_WARN << "Unsupported opcode " << static_cast<int>(opcode) << " from user "
                     << ctx.user_id << " conn_id=" << ctx.conn_id;
        }
    }  // end while
}
...
void CometServer::OnTextMessage(const TcpConnectionPtr &conn, ConnContext &ctx,
    ...
    // ========================================================================
    // 特殊处理：web_demo 心跳 ping -> pong
    // ========================================================================
    // web 端周期性发 {"type":"ping","ts":...}（见
    // web_demo/static/room_chat.html） 该消息不应转发到 logic，也不应返回
    // error，直接回 pong 即可。
    if (meta.msg_type == "ping" && meta.target_type == "control") {
        int64_t ts = meta.client_ts_ms;
        std::string pong_payload = "{\"type\":\"pong\",\"ts\":" + std::to_string(ts) + "}";
        conn->send(BuildWebSocketTextFrame(pong_payload));
        return;
    }
    ...
    // 步骤 3: 校验目标类型和 ID
    // ========================================================================
    // 支持 single_chat 和 room 两种目标类型
    // - single_chat: 一对一单聊
    // - room      : 房间/群聊消息（04_room_subset 新增，供 web_demo 使用）
    if ((meta.target_type != "single_chat" && meta.target_type != "room") || meta.target_id <= 0) {
        std::string frame = BuildWebSocketTextFrame(
            "{\"type\":\"error\",\"code\":400,\"message\":"
    ...
    auto status = logic_stub_->SendUpstreamMessage(&rpc_ctx, req, &rep);  //真正 发送
    if (!status.ok() || rep.error().code() != 0) {
        std::string msg = status.ok() ? rep.error().message() : status.error_message();
    ...
    std::string frame = BuildWebSocketTextFrame(
        ...
        "\",\"msg_seq\":" + std::to_string(rep.message().msg_seq()) + ",\"client_msg_id\":\"" +
        rep.message().client_msg_id() + "\"}");
    conn->send(frame);
}
...
// ============================================================================
void CometServer::OnMessage(const TcpConnectionPtr &conn, Buffer *buf, muduo::Timestamp ts) {
    // 忽略时间戳参数（本函数不需要）
    (void)ts;
    ...
    std::string conn_id;
    {
        ...
        if (it != user_conns_.end() && !it->second.empty()) {
            try {
                ...
                auto ctx = std::any_cast<ConnContext>((*it->second.begin())->getContext());
                conn_id = ctx.conn_id;
            } catch (...) {
                // 忽略异常（上下文不存在或类型不匹配）
            }
...
// - 支持多端登录：一个用户可以有多个 field（不同连接、不同 comet 节点）
...
//
// 示例：
// HSET user_connections:12345 comet-001-1703123456789-1 comet-001
// PEXPIRE user_connections:12345 60000
// ============================================================================
void CometServer::RegisterUserOnline(int64_t user_id, const std::string &conn_id) {
    // 参数校验
    ...
    // 执行 HSET 命令（统一写入：conn_id -> comet_id）
    reply = (redisReply *)redisCommand(ctx, "HSET %s %s %s", key.c_str(), field.c_str(),
                                       comet_id_.c_str());
...
void CometServer::RefreshUserTtl(int64_t user_id, const std::string &conn_id) {
    ...
    redisReply *reply = (redisReply *)redisCommand(ctx, "PEXPIRE %s %lld", key.c_str(),
                                                   static_cast<long long>(redis_ttl_ms_));
    if (reply) freeReplyObject(reply);  // 释放回复对象
}
...
void CometServer::RemoveUserRoute(int64_t user_id, const std::string &conn_id) {
    ...
    // 执行 HDEL 命令（删除统一格式：field=conn_id）
...
void CometServer::CheckIdleConnections() {
    ...
    std::vector<TcpConnectionPtr> to_close;
    {
        ...
        if (idle_scan_shards_ > 1) {
            idle_scan_shard_idx_ = (idle_scan_shard_idx_ + 1) % idle_scan_shards_;
        }
    }
    ...
    for (auto &c : to_close) {
        ...
        if (c->connected()) {
            LOG_WARN << "Close idle connection from user " << c->peerAddress().toIpPort();
