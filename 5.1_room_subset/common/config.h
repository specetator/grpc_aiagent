...
struct Config {
    // 日志配置
    // - log_level: TRACE/DEBUG/INFO/WARN/ERROR/FATAL（大小写不敏感）
    // - log_file: 为空则使用默认 logs/<program>.log；非空则重定向到指定文件
    ...
    int http_port{0};
    // logic HTTP 服务的 muduo IO 线程数（0/1 表示单线程；>1 可提升并发吞吐）
    int logic_http_threads{4};
