...
bool RedisConnectionPool::Validate(redisContext** ctx) {
    ...
    // 性能优化：
    // 旧实现：每次 Acquire 都发一次 PING 做网络校验，会在高 QPS 下把 Redis 压垮，
    // 导致尾延迟飙升（perf_client 表现为大量 WS 5s 超时）。
    //
    // 新策略：无网络校验。只要 hiredis ctx 没有显式错误，就认为连接可用；
    // 如果连接已断，后续 redisCommand 会设置 ctx->err，届时会被回收重建。
    if ((*ctx)->err) {
        redisFree(*ctx);
        *ctx = nullptr;
        return false;
