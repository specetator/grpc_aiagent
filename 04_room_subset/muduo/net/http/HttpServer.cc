...
bool HttpServer::onRequest(const TcpConnectionPtr& conn, HttpRequest& req) {
    ...
    if (syncProcessed) {
        ...
        LOG_DEBUG << "Sync request completed";
    } else {
        LOG_DEBUG << "Async request, waiting for response";
    }
